...
template <Enum16 E>
constexpr auto enum_to_int (E e) { return static_cast<std::underlying_type_t<E>> (e); }
...
inline constexpr void Assert (bool e, std::string_view msg = "", std::source_location loca = std::source_location::current ())
{
#ifdef _DEBUG
	if (!e)
	{
...
inline static [[nodiscard]] std::string base64_decode (const std::string& encoded_str)
{
	...
	if (decoded_length == 0)
		return { };
	return { decoded.data () , std::size_t (decoded_length) };
}
